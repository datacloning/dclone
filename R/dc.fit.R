...
#' 
#' \code{\link{jags.fit}} or \code{\link{bugs.fit}} is iteratively used to fit
#' a model with increasing the number of clones.
#' 
#' The function fits a JAGS/BUGS model with increasing numbers of clones, as
#' supplied by the argument \code{n.clones}. Data cloning is done by the
#' function \code{dclone} using the arguments \code{multiply} and
#' \code{unchanged}. An updating function can be provided, see Examples.
#' 
...
#' @param update Character, the name of the list element(s) in the \code{data}
#' argument that has to be updated by \code{updatefun} in each iteration. This
#' usually is for making priors more informative, and enhancing convergence.
#' See Details and Examples.
#' @param updatefun A function to use for updating named elements in
#' \code{data}. It should take an 'mcmc.list' object as 1st argument, 2nd
#' argument can be the number of clones. If \code{legth(update) > 1} the
#' function must return a named list so that \code{data[update]} can be
#' updated. See Details and Examples.
...
#' inspection. Otherwise only the MCMC list corresponding to highest number of
#' clones is returned with summary statistics for the rest.
